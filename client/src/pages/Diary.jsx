  ...
  return (
    <div>
      ...
      <div>
        {entries.map((entry) => (
          <div key={entry.id}>
            ...
            <div>{entry.issue}</div>
            <div>{entry.issueDescription}</div>
      ...
      <button style={{ fontSize: "24px", padding: "2em" }}>
        <Link to={`/diaryEntry?eid=${eid}&did=${did}`}>Add diary Entry</Link>
      </button>
      {/* <button style={{ fontSize: '24px', padding: '2em' }} onClick={() => { navigate("/diaryUpdate");}}> Update diary Entry</button> */}
      <button
        style={{ fontSize: "24px", padding: "2em" }}
        onClick={() => {
          handleDelete();
        }}
      >
        <Link to={`/diarylist?eid=${eid}`}>Delete diary</Link>
      </button>
    </div>
  );
}
